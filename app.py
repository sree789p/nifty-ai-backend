...
def build_drivers(f):
    ...
    drivers.append({
        ...
        "label": f"Nifty {'up' if n >= 0 else 'down'} {abs(round(n,2))}% | RSI {round(f['nifty_rsi'],0)}",
        "detail": f"{round(f['nifty_change_1w'],2)}% weekly"
    })
    
    nq, sp = f["nasdaq_change"], f["sp500_change"]
    drivers.append({
        "icon": "📈" if nq >= 0 else "📉",
        "tag": "pos" if nq >= 0 else "neg",
        "label": f"Nasdaq {round(nq,2)}% | S&P {round(sp,2)}%",
        "detail": "US markets"
    })
    
    nk, hs = f["nikkei_change"], f["hangseng_change"]
    drivers.append({
        "icon": "🌏",
        "tag": "pos" if (nk + hs) >= 0 else "neg",
        "label": f"Nikkei {round(nk,2)}% | HangSeng {round(hs,2)}%",
    ...
    drivers.append({
        ...
        "label": f"India VIX {round(vix,1)} | US VIX {round(f['us_vix'],1)}",
...
def run_backtest():
    try:
        ...
        return {"accuracy": accuracy, "total_days": total, "correct": correct, "last_10": last_10, "summary": f"Model correct {accuracy}% over 3 months"}
    except Exception as e:
        traceback.print_exc()
        return {"error": str(e)}
...
def build_telegram_msg(data, session):
    ...
    chg_val = nifty.get("change_pct", 0) or 0
    rsi = nifty.get("rsi", 0) or 0
    india_vix = mkt.get("india_vix", {}).get("price", 0) or 0
    
    regime_map = {"volatile": "HIGH VOL", "trending": "TRENDING", "oversold": "OVERSOLD", "overbought": "OVERBOUGHT", "range": "RANGE"}
    ...
    driver_lines = ""
    for d in drivers[:4]:
        driver_lines += f"{d.get('icon','')} {d.get('label','')}\n"
        
    msg = f"Nifty AI v2.0\n{session_text} | {date_str}\n\n"
    msg += f"Nifty: {price} ({'up' if chg_val >= 0 else 'down'} {abs(chg_val)}%)\n"
    msg += f"RSI: {rsi} | VIX: {india_vix}\n"
    msg += f"Regime: {regime_text}\n"
    msg += f"Signal: {direction} ({confidence}%)\n\n"
    msg += f"Drivers:\n{driver_lines}\n"
...
def send_telegram(message):
    ...
    try:
        res = requests.post(url, json={"chat_id": CHAT_ID, "text": message}, timeout=30)
...
def api_signal():
    try:
        market = {name: get_cached(ticker) for name, ticker in TICKERS.items()}
        ...
        result = {
            ...
            "market": {k: market[k] for k in ["nifty", "banknifty", "nasdaq", "nikkei", "hangseng", "usd_inr", "crude", "india_vix"]}
        }
...
def api_telegram():
    try:
        ...
        ok, res = send_telegram(build_telegram_msg(data, session))
        return jsonify({"status": "ok" if ok else "error", "message": res})
    except Exception as e:
        traceback.print_exc()
        return jsonify({"status": "error", "message": str(e)}), 500
